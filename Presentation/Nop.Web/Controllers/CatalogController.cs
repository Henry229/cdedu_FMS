...
namespace Nop.Web.Controllers
{
    public partial class CatalogController : BasePublicController
    {
        ...
        public ActionResult Checkpoint2(string stud_id, string password, string stud_firstname, string stud_lastname, string stud_email, string grade, string csuser)
        {
            if (String.IsNullOrEmpty(stud_id) || String.IsNullOrEmpty(password) || String.IsNullOrEmpty(stud_email))
            {
                return Content("02|Invalid field error");
            }

            string result = _productTagService.CreateUser(stud_id, password, stud_firstname, stud_lastname, stud_email, grade, csuser);
            //invoke stored procedure

            return Content(result);
        }

        public ActionResult Checkpoint4(string stud_id, string guid)
        {
            if (String.IsNullOrEmpty(stud_id) || String.IsNullOrEmpty(guid))
            {
                return Content("02|Invalid field error");
            }

            string result = _productTagService.InsertSales(stud_id, guid);
            //invoke stored procedure
        ...
        public ActionResult Checkpoint6(string stud_id, string ipaddress)
        {
            if (String.IsNullOrEmpty(stud_id) || String.IsNullOrEmpty(ipaddress))
            {
                return Content("02|Invalid field error");
            }

            string result = _productTagService.InsertLogHistory(stud_id, ipaddress);
            //invoke stored procedure
